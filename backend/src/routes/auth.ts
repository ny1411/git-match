...
router.post("/signup", async (req, res) => {
	try {
		...
		const { fullName, email, githubProfileUrl, password }: SignupRequest =
			req.body;
		...
		if (!fullName || !email || !githubProfileUrl || !password) {
			const response: AuthResponse = {
				success: false,
				message:
					"All basic fields are required: fullName, email, githubProfileUrl, password",
			};
		...
		const userProfile: UserProfile = {
			...
			goal: null,
			aboutMe:
				"Passionate developer looking to connect with like-minded tech enthusiasts!", // Default
...
router.post("/token-info", (req, res) => {
	try {
		...
		if (parts.length < 2)
			return res
				.status(400)
				.json({ success: false, message: "Invalid JWT format" });
